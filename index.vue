      ...
      methods: {
        async getTagsFromUrl(url, tagSelector) {
          try {
            ...
            const tags = Array.from(new DOMParser().parseFromString(response.data, 'text/html').querySelectorAll(tagSelector)).map(element => element.textContent.trim());
            return tags;
          } catch (error) {
            console.error(error);
            return null;
        ...
        async getCrawledData() {
          try {
            ...
            const links = Array.from(mainResponse.data.querySelectorAll('body.new-resource-list .filter-tags-row .tag-slider--list li a, body.new-resource-list .no-results--popular .tag-slider--list li a'))
